...
# create batch processing function for segmentation/skeletonization (folder of files as input)
...
# create batch processing function for segmentation/skeletonization (single file as input, for parallel processing on cluster)
def batch_PNN_seg_skel_PARALLEL(file_path: str,
                        # file_type: str,
                        out_path: str,
                        gaus_sigma: Union[float, int],
                        med_size: Union[float, int],
    ...
    -----------
    file_path: str
        location of the single image file to process, including the file name and extension
    out_path: str
        location where segmentation/skeleton images and data tables will be saved; if the path does not exist, it will be created for you
    gaus_sigma: float or int
        sigma value for Gaussian smoothing filter; set to 0 to skip this step
    ...
    # This is handled in the .sh script that calls this function for parallel processing, but we can keep this here just in case    
    # if not Path.exists(Path(out_path)):
    #     Path.mkdir(Path(out_path))
    #     print(f"Output file path not found. Creating: {out_path}")
    ...
    # changing the file_path to be a path to a single file
    f = Path(file_path)
